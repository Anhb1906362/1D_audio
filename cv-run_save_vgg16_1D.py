        ...
        for filename in files:
            if filename.endswith('.txt'):
                ...
                with open(file_path, 'r') as file:
                    ...
                    data.extend([float(line.strip()) for line in file])
    return np.array(data)  # Convert the combined data to a numpy array

print('chay qua day fc')

# Load dữ liệu huấn luyện và kiểm tra
train_ds = load_data_from_folder(folder_train)
test_ds = load_data_from_folder(folder_test)
y_train = np.zeros(train_ds.shape[0])
y_test = np.ones(test_ds.shape[0])


print('chay qua day fc1')

# Reshape the data for 1D input
...
if nkfold in [0,1]:
    ...
    if mod == 'vgg':
        ...
        print('chay qua day fc')
        model = Sequential()       
        model.add(InputLayer(input_shape=(110250,)))  
        model.add(Dense(units=num_class, activation="softmax"))
        opt = Adam(lr=learning_rate)
        model.compile(optimizer=opt, loss=keras.losses.categorical_crossentropy, metrics=['accuracy'])
        print('fc')
        ...
        model.add(Conv2D(filters=128, kernel_size=(3, 3), activation='relu', input_shape=(IMAGE_LEN, IMAGE_LEN, 3)))
        model.add(MaxPool2D(pool_size=(2, 2)))
